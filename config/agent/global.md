...
* **VC operations are off by default** - only proceed when I explicitly instruct.
* Without explicit instruction: no push, no open/close/comment on PRs or issues.
...
* Do not run destructive ops (`git reset --hard`, `git push --force`, `git checkout --`, `jj abandon` / `jj undo` / `jj squash` / `jj rebase`) unless I explicitly instruct or a skill explicitly calls for it.
